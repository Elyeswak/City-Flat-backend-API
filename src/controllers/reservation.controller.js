...
/*export async function httpGetMyReservations(req, res) {
  ...
  try {
    ...
    const filteredReservations = reservations.filter(
      (reservation) => reservation.Order.User.id === foundUser.id
    );
...
export async function httpGetMyReservations(req, res) {
   ...
   try {
     ...
     const filteredReservations = reservations.filter((reservation) => {
       return reservation.Order && reservation.Order.User && reservation.Order.User.id === foundUser.id;
     });
      ...
      createCustomer(foundUser)
        .then((customerId) => {
          ...
          if (
            ...
            return res
              .status(400)
              .json({ error: "Card details are incomplete" });
          }
                  ...
                  addCard(customerId, token.id)
                    .then((card) => {
                      ...
                      if (card) {
                                      ...
                                      .then((paymentIntent) => {
                                        reservationDb
                                          ...
                                          .then((result) => {
                                            ...
                                            updateBookedDates(
                                              newReservation.Order.appartment
                                                .id,
                        ...
                        return res
                          .status(400)
                          .json({ error: "No card found for the customer." });
                      }
                    })
                    .catch((error) =>
    ...
    .then((foundOrder) => {
      if (!foundOrder) {
        ...
        if (user.id == foundOrder.User._id) {
          orderDb
            ...
            .then((result) => {
              ...
              const notification = {
                user: user.id,
                message:
                  "You have declined the reservation for :" +
          ...
          res
            .status(500)
            .json({ error: "User does not correspond to the reservation" });
        }
      }
    })
...
export async function getAcceptedBookings(req, res) {
  ...
  const acceptedOrders = await orderDb.find({
    appartment: apartment.id,
